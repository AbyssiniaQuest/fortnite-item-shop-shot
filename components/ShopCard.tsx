...
export function ShopCard({ item, birrPerVbuck }: ShopCardProps) {
  ...
  return (
    <article className="overflow-hidden rounded-lg border border-white/10 bg-slate-950/75 shadow-[0_14px_35px_rgba(0,0,0,0.28)]">
      <div className="relative grid aspect-square place-items-center bg-[radial-gradient(circle_at_50%_25%,rgba(34,211,238,0.22),transparent_35%),linear-gradient(145deg,rgba(30,41,59,0.95),rgba(2,6,23,0.96))]">
        ...
        />
        <span className="absolute left-2 top-2 rounded bg-black/55 px-2 py-1 text-[10px] font-black uppercase tracking-normal text-cyan-100">
          {item.rarity}
        </span>
      </div>
      ...
      <div className="grid gap-2 p-3">
        <div>
          ...
          <h3 className="line-clamp-2 min-h-10 text-sm font-black leading-tight text-white">{item.name}</h3>
          <p className="mt-1 line-clamp-1 text-[11px] font-semibold text-slate-400">{item.season}</p>
        </div>

        <div className="grid gap-1 rounded-md bg-white/[0.06] p-2 text-xs">
